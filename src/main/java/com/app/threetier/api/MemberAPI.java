...
public class MemberAPI {
    ...
    @Operation(summary = "이메일 중복 확인", description = "이메일 중복 확인해서 결과를 알려주는 서비스")
    @ApiResponse(responseCode = "200", description = "이메일 중복 확인 여부")
    @GetMapping("exist-email")
    public ResponseEntity<ApiResponseDTO> existMemberEmail(@RequestBody String memberEmail) {
         memberService.existMemberEmail(memberEmail);
        return ResponseEntity.status(HttpStatus.OK).body(ApiResponseDTO.of("회원 가입 완료"));
    }
...
//    })
    ...
    @Operation(summary = "회원 수정", description = "회원의 정보를 수정할 수 있는 API")
